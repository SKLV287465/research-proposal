...
clean:
	rm -f *.aux *.toc *.bbl *.blg *.dvi *.lo? *.pstex* *.eps *.cb *.brf msc/* \
		*.rel *.idx *.out *.ps *-diff.* *.mps *.diff *.cut tmp.* $(GreyFigs)

clobber: realclean

realclean: clean
	rm -rf *~ *.pdf *.tgz $(Bib) imgs-gr
	$(Status) | sed -n 's/ *deleted: *//p' | xargs $(Revert)
...
arxiv:	source
	${Q}ntargets=$$( echo $(Targets) | wc -w ) ; \
	if [ $$ntargets != 1 ]; then \
		echo >&2 Too many targets: $$ntargets ; exit $$ntargets; \
	fi
	${Q}rm -rf A
	${Q}mkdir A
	${Q}Base=$$( echo  $(Targets) ); \
	echo \"$$Base.tex\" ; \
	dir=$$(basename $$PWD) && \
		cp cleaner_config.yaml A/ && \
		cd A && \
		tar xzf ../$$dir.tgz && \
		echo mv $$dir/$$Base.tex $$dir/$$Base.tex-keep && \
		mv $$dir/$$Base.tex $$dir/$$Base.tex-keep && \
		rm -f $$dir/*.tex $$dir/*.pdf && \
		mv $$dir/$$Base.tex-keep $$dir/$$Base.tex && \
		sed < $$dir/$$Base.tex > $$dir/arxiv.tex -E \
		    -e 's/\\if(Draft|Review|Alpha|Final|CA)/\\iffalse/g' \
		    -e 's/\\(Draft|Review|Alpha|Final|CA)(true|false)//g' \
		    -e 's/\\ifhyperlinks/\\iftrue/g' \
		    -e 's/\\newif.*//g' \
		    -e '/\\docstatus/d' \
		    -e '/\\submNr/d' && \
		sed < $$dir/usenix.sty > $$dir/arxiv.sty -E \
		    -e 's/\\if(Draft|Review|Alpha|Final)/\\iffalse/g' && \
		rm $$dir/$$Base.tex  $$dir/usenix.sty && \
		mv $$dir/arxiv.tex $$dir/$$Base.tex && \
		mv $$dir/arxiv.sty $$dir/usenix.sty && \
		( cd $$dir && make $$Base.pdf </dev/null ) && \
		arxiv_latex_cleaner --config cleaner_config.yaml $$dir && \
		( cd *_arXiv && rm -f cleaner_config.yaml README export.bst changes.* )

tmp_all:
	@echo "=========================="
	@echo "Before starting to work on your paper/report, do the following:"
	@echo " - Read README.md"
	@echo " - Chose the document style (paper, report, simple, thesis)"
	@echo " - If you're using paper.tex, remove unneded configurations for readability"
	@echo " - Remove unneeded .tex .cls, .sty, .bst files"
	@echo " - In the Makefile, remove the tmp_all target and unneeded Target strings"
	@echo "=========================="
...
help:
	...
	@echo "'make arxiv' will create a stripped version for upload to arXiv etc"
	@echo "'make grey' will generate greyscale figures (for B&W readibility check)"
	@echo "'make gray' is 'make grey' for Americans ;-)"
	@echo "'make warn' will show LaTeX and BibTeX warnings"
